...
public class AssociateControllerTests {
	...
	private Associate mockAssociate(int id) {
		return new Associate(id, "salesID", "email@email.com", "John", "Doe", 
			   ...
			   new Batch(1, "salesID", "name", "skill", "location"), 
			   AssociateStatus.STAGING);
	}
	
	private AssociateDTO mockDTO(int id) {
		return new AssociateDTO(id, "salesID", "email@email.com", "John", "Doe", 
								1, 1, AssociateStatus.STAGING.toString());
	}
	
	/*
	 * Sanity Check
	 *  - if this fails, application context is not loaded
	 *    and all other tests should fail
	 */
	@Test
	...
	public void controllerSuccess() throws Exception{
		...
		this.mockmvc.perform(get("/associates").param("manager", "1"))
		.andDo(print())
	...
	/*
	 * Verifies HTTP Status for a null return value from the
	 * service layer
	 */
	@Test
	...
	/*
	 * Verifies HTTP Status for an empty list returned from
	 * the service layer
	 */
	@Test
	...
	public void controllerFailWrongParamType() throws Exception{
		...
		this.mockmvc.perform(get("/associates").param("manager", "string"))
		.andDo(print())
		.andExpect(status().isBadRequest());
	}
	
	/*
	 * Verifies HTTP Status where parameter name
	 * is not 'manager'
	 */
	@Test
	public void controllerFailWrongParamName() throws Exception{
		
		this.mockmvc.perform(get("/associates").param("associate", "1"))
		.andDo(print())
		.andExpect(status().isBadRequest());
	}
