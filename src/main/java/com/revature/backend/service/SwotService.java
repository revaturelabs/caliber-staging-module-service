...
public class SwotService {
   ...
   * int) as a parameter. Returns a List of Swots for that Associate.
   */
  ...
  /*
   * Creates a new AnalysisItem for a SWOT. Accepts the AnalysisItem as a
   * parameter. Returns true if successful, false otherwise.
   */
  ...
  /*
   * Updates an AnalysisItem. Takes the AnalysisItem object that needs to be
   * updated with the changed fields.
   * 
   * Prior to calling the repo, set the respective SWOT's lastModified field to
   * System.currentTimeMillis() and then save the object to the database to
   * update.
   * 
  ...
  /*
   * Deletes an AnalysisItem from a SWOT by its id. Only takes in the Id as an
   * integer as a parameter. Should return true if successful, false otherwise.
   */
  ...
  /*
   * Method to retrieve all SWOTs found in the database. Currently only used for
   * testing purposes.
   */
