...
public class ManagerBalancerImpl implements ManagerBalancer {
   ...
   * assigned to the same manager - each manager should end up with a (roughly)
  ...
  /**
   * Sorts the given 2D array of batches in decreasing order by size (largest
   * array to smallest array).
   * 
   * @param batches
   */
   ...
   */
  public void assignAssociatesEvenly(Map<Manager, Integer> managerMap, Associate[][] batches) {
  ...
  /**
   * Searches the given map of managers/integers for the manager key with the
   * smallest Integer value (that is, the manager with the least assigned
   * associates).
   * 
   * If there is a tie, the first tied Manager in the map's key set will be
   * returned. Given an empty map, returns null
   * 
   * @param managerMap
   * @return
   */
   ...
   * batches - the string is in the format <a, b,...z> where the values are the
   * lengths of each sub array, in the order they are in the array.
   * 
   * @param batches
   * @return
   */
