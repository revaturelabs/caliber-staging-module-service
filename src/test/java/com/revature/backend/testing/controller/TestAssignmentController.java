...
public class TestAssignmentController {
  ...
  public void testConvertToAssociate() {
    ...
    // the expected and actual output
    Associate expected = new Associate(salesforceId, email, firstName, lastName, null, null, null);
  ...
  /**
   * Generates some dummy data and makes sure it is processed correctly. Assumes
   * the convert methods work (as they are tested separately)
   */
  @Test
  public void testAddNewBatches() {
    ...
    ApiAssociateAssignment[] assignmentArray = new ApiAssociateAssignment[] { assignment };
    ...
    when(mockBatchRetriever.retrieveNewlyStagingBatches()).thenReturn(batchTemplateList);
  ...
  /**
   * Tests that the assignmentController properly detects that there are no new
   * batches.
   */
  @Test
