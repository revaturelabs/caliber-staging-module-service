...
public class ManagerTesting {
  ...
  @MockBean
  BackendRepository mockBackendRepository;
  ...
  /**
   * Tests that managerService behaves properly when the repo finds some
   * manager(s)
   */
  @Test
  ...
  void findAllManagerFail() {
    when(mockManagerRepository.findAllManagers()).thenThrow(new ArrayIndexOutOfBoundsException()); // arbitrary type
    List<Manager> m = managerService.getAllManagers();
    assertNull(m);
  }

  /**
   * Tests that managerService correctly builds the map of managers and the count
   * of their assigned associates
   * 
  ...
  public void testGetAllManagersAndAssociates() {
    ...
    when(mockBackendRepository.findAssociatesByManagerId(1)).thenReturn(m1Assocs);
    when(mockBackendRepository.findAssociatesByManagerId(2)).thenReturn(m2Assocs);
    when(mockBackendRepository.findAssociatesByManagerId(3)).thenReturn(m3Assocs);
  ...
  /**
   * Helper method that builds a list of dummy associates, assigned to the given
   * manager
   */
